...
import publicSale from "@/assets/images/publicSale.png";
...
function PublicSale() {
  const pid = 1; // 公售的pid，正式环境，公售1
  const [comingSoon, setComingSoon] = useState(true);
  const toast = useToast();
  const { t, i18n } = useTranslation("common");
  const { config } = useConfigContext();
  const { address, signer } = useContext(ConfigContext);
  ...
  const erc20Abi = useMemo(() => {
    ...
    return Erc20__factory.connect(config?.usdt, signer);
  }, [signer, config]);
